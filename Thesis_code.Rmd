...
validation_accuracy <- ggplot()+
  ...
  geom_hline(yintercept=0.99,linetype="dashed",col="red")+
...
validation_recall <- ggplot()+
  ...
  geom_hline(yintercept=0.987,linetype="dashed",col="red")+
...
## Conv v Green Roof
```{r final boxplot}
roofs_only_insects$outlier <- c(1:13)

for (i in c(1:13)) {
  if (roofs_only_insects$total_inv_adj[i] > 100){
    roofs_only_insects$outlier[i] <- 1
  }
  else {
    roofs_only_insects$outlier[i] <- 0
  }
}
roofs_only_insects$outlier <- as.logical(roofs_only_insects$outlier)

green_insects_only <- roofs_only_insects %>%
  filter(green_roof=="green_roof")

green_insects_only <- green_insects_only %>%
  mutate(Points_Type = ifelse(Type == "intensive", "Type1", "Type2"))

roofs_only_insects <- roofs_only_insects %>%
  group_by(green_roof) %>%
  mutate(Type.show1 = as.numeric(  
    between(total_inv_adj, 
            quantile(total_inv_adj)[2] - 1.5*IQR(total_inv_adj),
            quantile(total_inv_adj)[4] + 1.5*IQR(total_inv_adj)))) 

light_green <- rgb(200/255, 238/255, 130/255)

ggplot(roofs_only_insects,aes(x=green_roof,y=total_inv_adj, fill=green_roof)) +
  geom_boxplot(outlier.colour = NA) +
  geom_point(data = filter(roofs_only_insects, outlier), color = "chartreuse3", size = 3, shape = 16)+
  xlab("Type of Roof")+
  ylab("Total Number of Invertebrates")+
  theme(legend.position="none",
        axis.title.x = element_text(face="bold", family="Arial", colour="black", size=10),
  ...
  geom_jitter(aes(alpha=as.factor(Type.show1),color=green_roof),size = 3, shape = 16, show.legend=FALSE)+
...
```{r Listed 50 as binary}
roofs_only_insects$Listed_50_yn <- roofs_only_insects$Listed_50

for (i in c(1:13)){
  if (roofs_only_insects$Listed_50[i]==1){
    roofs_only_insects$Listed_50_yn[i] <- 1
  }
  if (roofs_only_insects$Listed_50[i]==2){
    roofs_only_insects$Listed_50_yn[i] <- 1
  }
}

roofs_only_insects$Listed_50_yn <- as.factor(roofs_only_insects$Listed_50_yn)
```
...
```{r}
confint(everything_vif_ap_insects2)
exp(0.0002785652*100)
exp(0.002670982*100)
...
```{r graph}
green_insects_only$outlier <- c(1:10)

for (i in c(1:10)) {
  if (green_insects_only$Redstart_minutes_even[i] > 100){
    green_insects_only$outlier[i] <- 1
  }
  else {
    green_insects_only$outlier[i] <- 0
  }
}

green_insects_only$outlier <- as.logical(green_insects_only$outlier)
...
ggplot(green_insects_only, aes(x=Type,y=Redstart_minutes_even,fill=Type))+
  geom_boxplot(outlier.colour = NA)+
  scale_fill_manual(name= "Type", values=c("intensive" = "darkolivegreen3","extensive" = light_green))+
  geom_point(data = filter(green_insects_only, outlier), aes(color = Points_Type), size = 3, shape = 16)+
  ...
  geom_jitter(aes(alpha=as.factor(Type.show),color=Points_Type),size = 3, shape = 16, show.legend=FALSE)+
  scale_alpha_manual(values = c(0,1))+
  scale_color_manual(name="Points_Type", values = c("Type1" = "darkgreen", "Type2" = "chartreuse3"))
...
all_detections$Time_numeric <- c(1:3920)
...
```{r}
roofs_only_insects_no_london$Listed_50_yn <- roofs_only_insects_no_london$Listed_50

for (i in c(1:12)){
  if (roofs_only_insects_no_london$Listed_50[i]==1){
    roofs_only_insects_no_london$Listed_50_yn[i] <- 1
  }
  if (roofs_only_insects_no_london$Listed_50[i]==2){
    roofs_only_insects_no_london$Listed_50_yn[i] <- 1
  }
}

roofs_only_insects_no_london$Listed_50_yn <- as.factor(roofs_only_insects_no_london$Listed_50_yn)
```
...
everything_vif_ap_insects <- glm.nb(formula = Redstart_minutes_even_rounded~Green_area_m2+Listed_50_yn+total_inv_adj,data=roofs_only_insects,link=log)
summary(everything_vif_ap_insects)

```
```{r}
mean(roofs_only_insects$total_inv_adj)
sd(roofs_only_insects$total_inv_adj)
range(roofs_only_insects$total_inv_adj)
```

```{r}

everything_vif_ap_insects <- glm.nb(formula = Redstart_minutes_even_rounded~Green_area_m2+Listed_50_yn,data=roofs_only_insects,link=log)
summary(everything_vif_ap_insects)
